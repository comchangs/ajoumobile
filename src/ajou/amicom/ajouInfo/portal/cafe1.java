...
public class cafe1 extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String menulist[] = new String[50]; // 처음 html에서 메뉴만 저장
		String printstr = ""; // 출력

		int listnum = 0;
		int linecompare = -1;

		// String cafestart = "<!-- 아향 테이블 시작 -->";// 식당시작
		String cafeend = "<!-- 아향 테이블 끝 -->"; // 식당 끝
		// String cafename1 = "dummy_string";// menulist에 처음부분 삽입
		String cafename2 = "lineend"; // menulist 마지막 부분 삽입
		String nomenu = "등록된 식단이 없습니다."; // 메뉴가 없을 경우
		...
		try {
			...
			for (;;) {
				...
				/*
				 * int check1 = linecapy.indexOf(cafestart); //식당시작부분 비교 후
				 * menulist 삽입 if (check1 != linecompare) { menulist[listnum] =
				 * cafename1; listnum += 1; }
				 */
				int check3 = linecapy.indexOf(nomenu); // 메뉴가 없을 경우 menulist 삽입
				...
				int check5 = linecapy.indexOf(time1); // 식사시간 비교 후 삽입
				...
				String startmenu = "	<td style='word-break:break-all'>"; // 메뉴 라인
																		// 시작부분
				String endmenu = "</td>"; // 메뉴 라인 끝부분
				...
				if (check9 != linecompare) {
					...
					String menumod4 = menumod3.replaceFirst(startmenu, ""); // 라인
																			// 시작
																			// 삭제
					String menumod5 = menumod4.replaceFirst(endmenu, ""); // 라인
																			// 끝
																			// 삭제

					menulist[listnum] = menumod5; // menulist 삽입
					listnum += 1;
				}

				int check2 = linecapy.indexOf(cafeend); // 식당끝 break
		...
		String menutime4 = "";
		/*
		 * for (allline = 0; allline < listnum; allline++) { //menulist에서 상단으로
		 * 필요없는 부분 넘기기 if (menulist[allline] == cafename1) { cutline = allline +
		 * 1; break; } }
		 */
