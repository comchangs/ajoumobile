...
public class haksaplan extends Activity {
	Context context;
	@Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        setContentView(R.layout.portal_haksaplan);  	
        context = this;
...
//	}
	...
	class MemberAdapter extends ArrayAdapter<MemberInfo>{
		...
		public View getView(int position, View convertView, ViewGroup parent){
			...
			if(items != null){
				((TextView)item_view.findViewById(R.id.small_type)).setText(items.get(position).date);
	...
	String DownloadXML() {
		...
		try {
			...
			// --------------------------
			URL url = new URL(
					"http://haksa.ajou.ac.kr/uni/uni/sreg/base/findSregSchoolScheduleInfoReg.action"); // URL
																										// 설정
			...
			parameter = new String(
					"<?xml version=\"1.0\" encoding=\"utf-8\"?>\n"
							+ "<root>\n"
							+ "<params>\n"
							+ "<param id=\"strUnivGrscCd\" type=\"STRING\">DS03</param>\n"
							+ "<param id=\"strYy\" type=\"STRING\">" + strYy
							+ "</param>" + "</params>\n" + "</root>");
	...
	private MemberAdapter PullParserFromXML(String data) {
		...
		boolean korDtNm = false;	// 날짜
    	boolean KorScheNm = false;	// 행사명
		...
		try{
        	...
        	while (parserEvent != XmlPullParser.END_DOCUMENT ){
        		...
        		case XmlPullParser.TEXT:
        			...
        			if (korDtNm) {
        				if(intro.DEBUG) Log.i(intro.LOG_TAG, "[Parse] korDtNm = " +  parser.getText() );
        				member.date = parser.getText();
        			}	
        			if (KorScheNm) {
        				if(intro.DEBUG) Log.i(intro.LOG_TAG, "[Parse] KorScheNm = " +  parser.getText() );  
        		...
        		case XmlPullParser.END_TAG:
        			tag = parser.getName();
        			if (tag.compareTo("korDtNm") == 0) {
        				korDtNm = false;
        			}
        			if (tag.compareTo("korScheNm") == 0) {
